...
def _fetch_google_news(
    ...
    for entry in getattr(feed, "entries", [])[: max(limit * 4, limit)]:
        ...
        if entry_dt is None and _exclude_unknown_published_at():
            continue
...
def fetch_global_breaking_news(
    ...
    hours = min(max(1, int(lookback_hours or 24)), _supporting_news_max_age_hours(48))
...
def fetch_related_news(
    ...
    lookback_hours = min(max(1, int(lookback_hours or 24)), _supporting_news_max_age_hours(48))
